...
class Gd_pengadaan extends Auth_Controller {
    ...
    public function reset() {
        ...
        if ($insert['id'] != 0) {
            ...
            if($this->user->cabang_id==1)
                $data2['tujuan'] = $this->Gdpengadaan_model->get_detail('id', $insert['id'], 'trx_pengadaan')->cabang_id;
            else
                $data2['tujuan'] = 1;
    ...
    private function __init_peng($insert) {
        ...
        if ($this->Gdpengadaan_model->insert($data, 'trx_pengadaan')) {
            ...
            // $no=$this->Gdpengadaan_model->insert_outgoing($data, 'head');
    ...
    public function delete_item() {
        ...
        if ($insert['id'] != 0) {
            ...
            $data2['jumlah'] = 1;
            if($this->user->cabang_id==1)
                $data2['tujuan'] = $this->Gdpengadaan_model->get_detail('id', $insert['id'], 'trx_pengadaan_detail')->cabang_id;
            else
                $data2['tujuan'] = 1;
            $data2['id_cabang'] = $this->user->cabang_id;
    ...
    public function approve_cabang() {
        ...
        foreach ($data as $row) {
            ...
            $data2[] = array('data' => $data_json, 'head_id' => '', 'primary_key' => $row, 'table_name' => 'trx_pengadaan');
            $jumlah++;
        }
        //Sinkronisasi
